...
const people = [
  { name: "Rommel Rivas", role: "Owner, The Blue Bus", bio: "Kiter, Venezuelan, Bonaire local. He took the keys of the Blue Bus in 2016 and never looked back. First one on the beach, last one to leave.", photo: rommelPhoto.src, hint: "rommel-rivas.jpg", dark: true },
  { name: "Carlos Lilue",  role: "Operations",          bio: "The one who makes the beach happen every day. Bus, boat, tents, chairs. Carlos sets up the world so everyone else can enjoy it.",                    photo: carlosPhoto.src, hint: "carlos-lilue.jpg", dark: false },
];
    ...
    <div className="min-h-screen">
      ...
      <section className="relative flex items-end justify-start overflow-hidden" style={{ minHeight: "80vh" }}>
        ...
        <div className="relative z-10 px-6 sm:px-10 lg:px-16 max-w-7xl w-full mx-auto pb-16 md:pb-28">
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            ...
            <motion.h1 variants={fadeUp} className="font-display font-black text-white uppercase tracking-tighter text-[clamp(56px,12vw,160px)] leading-[0.85]">The<br />Blue Bus</motion.h1>
            <motion.p variants={fadeUp} className="text-white/70 font-body uppercase tracking-[0.25em] mt-8 text-[clamp(11px,1.2vw,14px)]">The original. Still here. Still flying.</motion.p>
          </motion.div>
        </div>
      ...
      <section className="bg-background py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="max-w-3xl">
            ...
            <motion.p variants={fadeUp} className="font-body text-foreground leading-relaxed mb-6 text-[clamp(20px,2.5vw,28px)]" style={{ lineHeight: 1.4 }}>We are kiters who happen to run a school. Not the other way around.</motion.p>
            <motion.p variants={fadeUp} className="text-foreground/80 font-body text-lg leading-relaxed mb-4">Roan Jaspers put the first kite in the air on Atlantis Beach in 2001. Nobody had done it before in the Caribbean. In 2016, Rommel Rivas took the keys of the Blue Bus. Not as a businessman taking over an opportunity, but as a rider who had been part of this community for years and couldn't imagine it going anywhere else.</motion.p>
            <motion.p variants={fadeUp} className="text-foreground/80 font-body text-lg leading-relaxed">What has never changed in 25 years is the reason people show up. The wind. The water. The feeling you get when Atlantis Beach comes into view and the kites are already in the air.</motion.p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-background py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-2 border-foreground">
            {people.map((person, i) => (
              <motion.div key={person.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.6 }} className={`flex flex-col ${i === 0 ? "border-b-2 md:border-b-0 md:border-r-2 border-foreground" : ""}`}>
                <div className="relative overflow-hidden" style={{ height: "clamp(260px, 32vw, 440px)" }}>
                  {person.photo ? <img src={person.photo} alt={person.name} className="w-full h-full object-cover object-top" /> : <PhotoSlot height={9999} label={person.name} hint={person.hint} dark={person.dark} />}
                </div>
                <div className="p-8 flex flex-col" style={{ background: person.dark ? NAVY_DEEP : "white" }}>
                  ...
                  <h3 className={`font-display font-black uppercase tracking-tighter mb-4 text-[clamp(22px,2.8vw,36px)] leading-[0.9] ${person.dark ? "text-white" : "text-foreground"}`}>{person.name}</h3>
                  <p className={`font-body text-base leading-relaxed ${person.dark ? "text-white/80" : "text-foreground/80"}`}>{person.bio}</p>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="border-2 border-t-0 border-foreground">
            <div className="p-8 md:p-10 border-b-2 border-foreground">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
                ...
                <p className="text-foreground/70 font-body text-base leading-relaxed">Riders first, instructors second. They push the boat in and out of the water every session because they want you on the water as much as you do. Dutch, English, Spanish, Papiamentu. They speak your language.</p>
              </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4">
              {instructors.map((instructor, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className={`flex flex-col ${i < instructors.length - 1 ? "border-r-2 border-foreground" : ""}`}>
                  <div style={{ height: "clamp(160px, 20vw, 280px)", overflow: "hidden" }}>
                    {instructor.photo ? <img src={instructor.photo} alt={instructor.name} className="w-full h-full object-cover object-top" /> : <PhotoSlot height={9999} label="" hint={instructor.hint} dark={false} />}
                  </div>
                  <div className="px-5 py-4 bg-background border-t-2 border-foreground">
                    <p className="font-display font-black text-foreground/70 text-xs uppercase tracking-widest">{instructor.name}</p>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 items-center">
            <div>
              ...
              <motion.p variants={fadeUp} className="text-white font-body text-lg leading-relaxed mb-5">In 2014 the Venezuelan Kite Mafia had an idea. Gather the whole kite community on Atlantis Beach, costumes mandatory, rules optional. Seventy people showed up. The Kitemanera was born.</motion.p>
              <motion.p variants={fadeUp} className="text-white/70 font-body text-base leading-relaxed mb-8">It wasn't an event. It was proof of what this place is when everyone shows up together. That same spirit lives in everything we do here, every day.</motion.p>
              <motion.p variants={fadeUp} className="text-accent font-display font-black text-xs uppercase tracking-widest">Kitemanera is coming back. Atlantis Beach, Bonaire.</motion.p>
            </div>
      ...
      <section className="py-20 md:py-28 bg-primary text-center">
        <div className="max-w-3xl mx-auto px-6 sm:px-10 lg:px-16">
          <h2 className="font-display font-black text-white uppercase tracking-tighter mb-4 text-[clamp(40px,7vw,96px)] leading-[0.88]" style={{ letterSpacing: "-0.01em" }}>Find Us<br />at the Beach</h2>
          <p className="text-white/70 font-body mb-10 uppercase tracking-[0.2em] text-sm">Atlantis Beach, Bonaire. Every day from 9am.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/lessons" className="btn-cyan text-base w-full sm:w-auto">Book a Lesson</Link>
            <a href="https://wa.me/5997015483?text=Hi!%20I'd%20like%20to%20know%20more%20about%20Kiteboarding%20Bonaire" target="_blank" rel="noopener noreferrer" className="btn-outline-white text-base w-full sm:w-auto">WhatsApp Us</a>
          </div>
        </div>
      </section>
