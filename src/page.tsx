  ...
  return (
    <div style={{ background: "#F8F6F1" }}>
      ...
      <section className="relative overflow-hidden min-h-[100vh] flex items-end">
        ...
        {/* Gradient — transparent top half, hard dark ramp into bottom third */}
        ...
        {/* Text — pinned to very bottom, headline sits in the darkest zone */}
        <div className="relative z-10 w-full px-8 sm:px-14 lg:px-20 pb-10 md:pb-14">
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            ...
            <motion.p
              variants={fadeUp}
              className="font-body text-white/70 text-sm uppercase tracking-[0.25em] mb-8"
            >
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-4">
              <button
                ...
                style={{ background: "hsl(186,100%,42%)", borderRadius: "4px" }}
              >
                Book a Lesson
              </button>
              <Link
                ...
                style={{ border: "2px solid rgba(255,255,255,0.7)", borderRadius: "4px" }}
              >
                Our Story
              </Link>
            </motion.div>
      ...
      <section style={{ background: "#F8F6F1" }} className="py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-8 sm:px-14 lg:px-20">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-0 items-start">
            <div className="md:col-span-7">
              <motion.p variants={fadeUp} className="category-label mb-4">Bonaire&apos;s original kite school</motion.p>
              <motion.h2 variants={fadeUp} className="font-display font-black text-foreground uppercase tracking-tighter" style={{ fontSize: "clamp(42px, 6.5vw, 92px)", lineHeight: 0.91 }}>
                Learn to fly.<br />Find the wind.<br />Keep coming back.
              </motion.h2>
            </div>
            <div className="md:col-span-5 md:pl-14 md:pt-24">
              <motion.p variants={fadeUp} className="font-body text-base md:text-lg text-foreground/65 leading-relaxed mb-10">
              ...
              <motion.div variants={fadeUp} className="grid grid-cols-3 gap-4 pt-8" style={{ borderTop: "1px solid rgba(0,0,0,0.09)" }}>
                {[
                  ...
                  <div key={s.val}>
                    ...
                    <p className="font-body text-[11px] text-foreground/40 uppercase tracking-wide" style={{ whiteSpace: "pre-line" }}>{s.label}</p>
                  </div>
                ))}
              </motion.div>
            </div>
      ...
      {/* LESSONS SPLIT — flex row so both cols share real height */}
      <section className="flex flex-col md:flex-row" style={{ minHeight: "580px" }}>
        ...
        <div className="w-full md:w-1/2 flex flex-col justify-center px-10 md:px-16 py-16 md:py-20" style={{ background: "hsl(211,100%,14%)" }}>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.p variants={fadeUp} className="category-label mb-5">Lessons</motion.p>
            <motion.h2 variants={fadeUp} className="font-display font-black text-white uppercase tracking-tighter mb-6" style={{ fontSize: "clamp(36px, 3.5vw, 58px)", lineHeight: 0.93 }}>
              From zero<br />to riding<br />in 3 days.
            </motion.h2>
            <motion.p variants={fadeUp} className="font-body text-white/65 text-base leading-relaxed mb-10" style={{ maxWidth: "32ch" }}>
            ...
            <motion.div variants={fadeUp}>
              <Link href="/lessons" className="inline-block font-display font-black text-sm uppercase tracking-widest px-8 py-3.5 rounded-full text-white transition-all duration-200 hover:brightness-110" style={{ background: "hsl(186,100%,42%)" }}>
                View Lessons & Prices
              </Link>
            </motion.div>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-8 sm:px-14 lg:px-20">
          <div className="grid grid-cols-3 gap-4 md:gap-5">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: p.offset ? 50 : 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: p.delay, ease: [0.16,1,0.3,1] as const }}
                className={`relative overflow-hidden rounded-lg ${p.offset ? "mt-10 md:mt-16" : ""}`} style={{ aspectRatio: "3/4" }}>
                <img src={p.src} alt={p.alt} className="absolute inset-0 w-full h-full object-cover" />
              </motion.div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mt-14 pb-20 md:pb-28" style={{ borderTop: "1px solid rgba(0,0,0,0.08)", paddingTop: "2rem" }}>
            <div>
              <p className="category-label mb-2">Also available</p>
              <h3 className="font-display font-black text-foreground uppercase tracking-tighter" style={{ fontSize: "clamp(28px, 4vw, 52px)" }}>Premium gear rentals</h3>
            </div>
            <Link href="/rentals" className="inline-block font-display font-black text-sm uppercase tracking-widest px-8 py-3.5 rounded-full border-2 border-foreground text-foreground transition-all duration-200 hover:bg-foreground hover:text-background flex-shrink-0">
              Rent Equipment
            </Link>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden" style={{ background: "hsl(211,100%,14%)" }}>
        <img src={atlantisPhoto.src} alt="Atlantis Beach Bonaire kite spot" className="absolute inset-0 w-full h-full object-cover opacity-25" />
        <div className="relative z-10 max-w-7xl mx-auto px-8 sm:px-14 lg:px-20 py-20 md:py-28">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center">
            <div>
              <motion.p variants={fadeUp} className="category-label mb-5">Where we ride</motion.p>
              <motion.h2 variants={fadeUp} className="font-display font-black text-white uppercase tracking-tighter" style={{ fontSize: "clamp(40px, 5.5vw, 76px)", lineHeight: 0.91 }}>
                Atlantis Beach.<br />Best flat water<br />on the planet.
              </motion.h2>
            </div>
            <div>
              <motion.p variants={fadeUp} className="font-body text-white/70 text-base md:text-lg leading-relaxed mb-8">
              ...
              <motion.div variants={fadeUp}>
                <button onClick={() => setWeatherOpen(true)} className="inline-block font-display font-black text-sm uppercase tracking-widest px-8 py-3.5 rounded-full border border-white/40 text-white transition-all duration-200 hover:bg-white/10 hover:border-white/70">
                  Live Wind Forecast
                </button>
              </motion.div>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="px-8 sm:px-14 lg:px-20 mb-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              ...
              <h2 className="font-display font-black text-foreground uppercase tracking-tighter" style={{ fontSize: "clamp(34px, 5vw, 64px)", lineHeight: 0.91 }}>
                Stories from<br />the water
              </h2>
            </div>
            <a href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 font-display font-black text-xs uppercase tracking-widest px-6 py-3 rounded-full border border-foreground/25 text-foreground hover:border-foreground transition-all self-start sm:self-auto flex-shrink-0">
              <div className="flex gap-0.5">{[...Array(5)].map((_, j) => <Star key={j} className="w-3 h-3 text-accent fill-current" />)}</div>
              5.0 on Google
            </a>
          </div>
          <div className="flex gap-5 overflow-x-auto px-8 sm:px-14 lg:px-20 pb-4 hide-scrollbar snap-x snap-mandatory">
            {testimonials.map((t, i) => (
              <div key={i} className="snap-start flex-shrink-0 w-[300px] md:w-[340px] flex flex-col justify-between p-7 rounded-2xl" style={{ background: "#fff", boxShadow: "0 1px 16px rgba(0,0,0,0.06)" }}>
                <div>
                  <div className="flex gap-0.5 mb-5">{[...Array(5)].map((_, s) => <Star key={s} className="w-3.5 h-3.5 text-accent fill-current" />)}</div>
                  <p className="font-body text-foreground/75 text-sm leading-relaxed italic mb-5">&ldquo;{t.quote}&rdquo;</p>
                </div>
            ...
            <a href={GOOGLE_REVIEWS_URL} target="_blank" rel="noopener noreferrer" className="snap-start flex-shrink-0 w-[180px] flex flex-col items-center justify-center gap-3 p-7 rounded-2xl border border-dashed border-foreground/15 hover:border-accent hover:bg-accent/5 transition-all group">
              <div className="flex gap-0.5">{[...Array(5)].map((_, s) => <Star key={s} className="w-4 h-4 fill-current text-foreground/20 group-hover:text-accent transition-colors" />)}</div>
              <p className="font-display font-black text-foreground/35 group-hover:text-foreground text-[11px] uppercase tracking-widest text-center transition-colors">Leave a review</p>
            </a>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "hsl(211,100%,16%)" }} className="pt-20 md:pt-24 pb-0 relative">
        <div className="max-w-7xl mx-auto px-8 sm:px-14 lg:px-20 pb-20 md:pb-28">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-10">
            <div>
              ...
              <h2 className="font-display font-black text-white uppercase tracking-tighter" style={{ fontSize: "clamp(38px, 6vw, 84px)", lineHeight: 0.91 }}>
                Book your<br />lesson today.
              </h2>
            </div>
            <div className="flex flex-col sm:flex-row md:flex-col gap-3 flex-shrink-0">
              <button onClick={() => setBookingOpen(true)} className="font-display font-black text-sm uppercase tracking-widest px-10 py-4 text-white transition-all duration-200 hover:brightness-110 whitespace-nowrap" style={{ background: "hsl(186,100%,42%)", borderRadius: "4px" }}>
                Book a Lesson
              </button>
              <Link href="/rentals" className="font-display font-black text-sm uppercase tracking-widest px-10 py-4 text-white transition-all duration-200 hover:bg-white/10 whitespace-nowrap text-center" style={{ border: "2px solid rgba(255,255,255,0.45)", borderRadius: "4px" }}>
                Rent Equipment
              </Link>
            </div>
          </div>
        ...
        {/* Wave — bleeds into footer's navy bg */}
